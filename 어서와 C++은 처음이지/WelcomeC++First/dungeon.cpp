...
public:
	...
	void move(char d)
	{
		...
		//cout << "x : " << x<<endl;
		//cout << "y : " << y << endl;
		if (d == 'a' || d == 'd') x += rand() % 1 - 2;
		else if (d == 'w' || d == 's') y += rand() % 1 - 2;
		cout << "x : " << x << endl;
		cout << "y : " << y << endl;
...
int main()
{
	...
	while (true)
	{
		...
		if (list[0]->getX() == list[1]->getX() && list[2]->getY() == list[2]->getY())
		{
			cout << "괴물에게 잡혔습니다!" << endl;
			cout << "게임을 종료합니다" << endl;
			return 0;
		}


		if (list[0]->getX() == list[1]->getX() && list[0]->getY() == list[1]->getY())
		{
			cout << "보물을 찾았습니다!" << endl;
			cout << "게임을 종료합니다" << endl;
			return 0;
